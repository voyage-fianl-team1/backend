...
public interface ChatRepository extends JpaRepository<Chat,Long> {
    @Query("SELECT new com.sparta.matchgi.dto.ChatResponseDto(c.id,u.nickname,u.profileImgUrl,c.message,c.createdAt) " +
            "FROM Chat c  " +
            "JOIN c.user u " +
            "WHERE c.room = :room" +
            " ORDER BY c.id DESC ")
    List<ChatResponseDto> showChatsFirst(Room room, Pageable pageable);
